...
#define d 		 8
    ...
    /* Bubble, Quick */
#define sortelements 	 300000
#define srtelements 	 10000
...
void Move (s1,s2) int s1, s2; {
    Push(Pop(s1),s2);
    movesdone=movesdone+1;
}

void tower(i,j,k) int i,j,k; {
    int other;
    if ( k==1 ) {
	Move(i,j);
    } else {
...
void Try(i, q, a, b, c, x) int i, *q, a[], b[], c[], x[]; {
    int j;
    j = 0;
    *q = false;
    while ( (! *q) && (j != 8) ) {
...
void Queens () {
    int i;
    for ( i = 1; i <= 100000; i++ ) Doit();
}

/* Multiplies two integer matrices. */

void Initmatrix ( m ) int m[rowsize+1][rowsize+1]; {
    int temp, i, j;
    for ( i = 1; i <= rowsize; i++ ) {
	for ( j = 1; j <= rowsize; j++ ) {
	    temp = Rand();
...
void Innerproduct( result,a,b, row,column)
...
void rInitmatrix ( m ) float m[rowsize+1][rowsize+1]; {
    int temp, i, j;
    for ( i = 1; i <= rowsize; i++ ) {
	for ( j = 1; j <= rowsize; j++ ) {
	    temp = Rand();
    ...
    /* computes the inner product of A[row,*] and B[*,column] */
void rInnerproduct(result,a,b,row,column)
...
void Quicksort( a,l,r) int a[], l, r; {
    int i,j,x,w;
...
void CreateNode (t,n) struct node **t; int n; {
    *t = (struct node *)malloc(sizeof(struct node)); 
    ...
    /* insert n into tree */
void Insert(n, t) int n; struct node *t; {
    if ( n > t->val ) {
	if ( t->left == nil ) {
	    CreateNode(&t->left,n);
	} else {
...
void Printcomplex(  arg1, arg2, zarray, start, finish, increment)
...
void Uniform11(iy, yfl) int *iy; float *yfl; {
    *iy = (4855* *iy + 1731) & 8191;
    *yfl = *iy/8192.0;
}

void Exptab(n, e) int n; struct complex e[]; {
...
printf("\n");
for ( i = 1; i <= 10; i++ ) printf("%8.0lf", xtimes[i]);
printf("\n");
